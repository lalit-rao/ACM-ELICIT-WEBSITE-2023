...
const RteEventBoxes = ({events, selectedDate, setSelectedDate, setActiveBox, activeBox, isEventHighlighted}) => {
    ...
    const [clickedOnce, setClickedOnce] = useState(null);
    // Event data
    const eventData = [
        {
    ...
    const handleClick = (event, index) => {
        if (clickedOnce === index) {
            // If it's the second click, open the popup
            setPopupOpen(true);
            setCurrentEvent(eventData[index]);
            setClickedOnce(null); // Reset click state
        } else {
            // First click: reveal the event poster
            setClickedOnce(index);
            setActiveBox(index);
    ...
    return (
        <div
            className="event-container flex flex-wrap justify-center align-middle items-center gap-10 p-6 max-w-[100%] mx-auto">
